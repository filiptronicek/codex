...
#[test]
fn thread_candidate_collection_honors_scan_budget() {
    for layout in [ThreadListLayout::NestedByDate, ThreadListLayout::Flat] {
        ...
        for index in 0..100_u128 {
            File::create(root.join(format!(
                "rollout-2025-01-01T12-00-00-{}.jsonl",
                Uuid::from_u128(index + 1)
            )))
            .unwrap();
        }
        ...
        assert_eq!(
            crate::list::collect_thread_candidate_count_for_test(
                collection_root,
                layout,
                /*candidate_limit*/ 64,
            )
...
#[tokio::test]
async fn created_at_thread_list_stops_after_first_matching_batch() {
    let temp = TempDir::new().unwrap();
    ...
    assert_eq!(page.num_scanned_files, 51);
    assert_eq!(work.rollout_opens, page.num_scanned_files);
    assert_eq!(work.session_meta_records, page.num_scanned_files);
    assert_eq!(work.full_head_summaries, page.num_scanned_files);
}
