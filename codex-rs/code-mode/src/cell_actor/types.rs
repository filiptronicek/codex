...
impl CellState {
    ...
    pub(crate) fn request_termination(&self) -> CellEventFuture {
        ...
        match std::mem::replace(&mut *phase, CellPhase::Tombstone) {
            ...
            CellPhase::Completed {
                ...
                let event = prepend_pending_output(event, pending_yield_items);
    ...
    pub(crate) fn deliver_completion(
        ...
        let mut phase = self
            ...
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let (pending_yield_items, event) =
            match std::mem::replace(&mut *phase, CellPhase::Tombstone) {
                CellPhase::Completed {
                    pending_yield_items,
                    event,
                } => (pending_yield_items, event),
                previous => {
                    *phase = previous;
    ...
    pub(crate) fn finish_termination(&self, event: CellEvent) -> Option<CellEvent> {
        ...
        let observer_event = match std::mem::replace(&mut *phase, CellPhase::Tombstone) {
            ...
            CellPhase::Completed {
                pending_yield_items,
                event,
            } => Some(prepend_pending_output(event, pending_yield_items)),
...
fn prepend_pending_output(
    event: CellEvent,
    pending_yield_items: Option<Vec<OutputItem>>,
) -> CellEvent {
    let Some(mut pending_yield_items) = pending_yield_items else {
        return event;
    };
    match event {
        CellEvent::Yielded { mut content_items } => {
            pending_yield_items.append(&mut content_items);
            CellEvent::Yielded {
                content_items: pending_yield_items,
            }
        }
        CellEvent::Pending {
            mut content_items,
