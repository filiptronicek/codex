...
- `permissionPreset/list` — experimental; list the complete resolved permission-preset catalog for an optional `cwd`, using cursor pagination. Each preset includes a stable `id`, exhaustive `kind`, optional `permissionProfileId` and `description`, resolved sandbox/approval/reviewer settings, and structured availability metadata. The response also identifies one allowed `defaultPresetId` and its `defaultSource`. Disallowed and equivalently resolved presets remain in the catalog so clients can preserve explicit selection identity without reconstructing permission policy.
